  ...
  return (
    ...
    >
      <div className="bg-white/95 backdrop-blur-3xl rounded-t-[3rem] border-t border-white/60 h-full flex flex-col shadow-[0_-20px_100px_-20px_rgba(0,0,0,0.1)]">
        ...
        >
          <div className="w-10 h-1 bg-muted/40 rounded-full mb-3 md:mb-4" />
          <div className="flex items-center gap-3 text-[8px] md:text-[9px] font-bold tracking-[0.25em] uppercase text-muted-foreground/80">
        ...
        >
          {places.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-10 opacity-60">
              <p className="text-lg md:text-xl font-headline font-bold text-primary mb-2">Finding more magic...</p>
              <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-widest">Try a different search or category.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8 py-4">
              {places.map((place) => (
                <PlaceCard 
                  key={place.id} 
                  place={place} 
