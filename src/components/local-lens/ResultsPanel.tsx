    ...
    >
      <div className="bg-white/95 backdrop-blur-3xl rounded-t-[2.5rem] md:rounded-t-[3rem] border-t border-white/60 h-full flex flex-col shadow-[0_-20px_100px_-20px_rgba(0,0,0,0.1)]">
        ...
        <div 
          ref={panelRef}
          className={cn(
            "flex-1 overflow-y-auto no-scrollbar px-4 md:px-12 pb-16 transition-opacity duration-300",
            isExpanded ? "opacity-100" : "opacity-0 pointer-events-none"
          )}
        >
          {places.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-10 opacity-60">
              <p className="text-base md:text-xl font-headline font-bold text-primary mb-1 md:mb-2">Finding more magic...</p>
              <p className="text-[8px] md:text-[10px] text-muted-foreground font-medium uppercase tracking-widest">Try a different search or category.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-8 py-2 md:py-4">
              {places.map((place) => (
                <PlaceCard 
                  key={place.id} 
                  place={place} 
